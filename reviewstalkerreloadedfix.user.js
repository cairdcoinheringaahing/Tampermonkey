// ==UserScript==
// @name        Review Stalker Reloaded (temp fix)
// @namespace   com.tuggy.nathan
// @description Reloads specified Stack Exchange review pages, opening tasks as they show up
// @include     https://codegolf.stackexchange.com/review*
// @include     https://codegolf.meta.stackexchange.com/review*
// @exclude     //stats$/
// @exclude     //history($|\?.+$)/
// @version     1.9.04
// @grant       GM_openInTab
...
var NlNumAvailable = document.querySelectorAll("div:not(.mb2) .fs-body3[title]");
var NNumAll = document.querySelectorAll(".fs-body3").length;
...
Q:for (let NNumAvailable of NlNumAvailable) {
    ...
    let NLnkAvailable = NNumAvailable.parentNode.parentNode.querySelector(".fs-body3 > a");
...
function CreateHeaders() {
  ...
  ElemOptionsButton.addEventListener("click", function (e) {
    ...
    console.log("Hi Redwolf!", 2, ElemHeader, ElemOptionsButton);
...
if (BInQueue) {
  ...
  if (BForceSingleTab && BStackPresent(SttFetch)) {
    ...
    console.log("Hi Redwolf!", 3, NNumAll);
...
if (LHrefToOpen.length > 0 && BForceSingleTab) {
...
else if (LHrefToOpen.length > 0 || location.href.includes("TESTING")) {
  ...
  if (BTabForQueue) {
    CheckNextPage();
      ...
      var play = function() {
          ...
          var g = ac.createGain();
          o.frequency.value = 216 * Math.SQRT2;
          o.type = "sawtooth";
          o.connect(g);
          g.connect(ac.destination);
          g.gain.value = 1;
          o.start();
          g.gain.exponentialRampToValueAtTime(0.00001, ac.currentTime + 0.75);
          setTimeout(() => ac.close(), 750);
      };
  ...
  if (!BInQueue) {
      console.log("Hi Redwolf!", 1, ElemOptionsButton);
